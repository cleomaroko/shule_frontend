...
export default defineConfig(({ mode }) => {
  ...
  // Production on dira365.com is served at `/`. Local `npm run dev` is also
  // `/`. Docker passes VITE_BASE_PATH=/. The IP vhost keeps `/api` on the
  // backend and should not host this SPA at `/`.
